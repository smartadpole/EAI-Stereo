...
from eai_stereo import EAIStereo, autocast
...
def validate_things(model, iters=32, mixed_prec=False):
    ...
    for val_id in tqdm(range(len(val_dataset))):
        ...
        mean_epe = epe[val].mean().item()
        if not np.isnan(mean_epe):
            out = (epe > 1.0)
            epe_list.append(mean_epe)
            out_list.append(out[val].cpu().numpy())
            # print(mean_epe)
    print("avg time:", str(time_all / len(val_dataset)))
    epe_list = np.array(epe_list)
    out_list = np.concatenate(out_list)
...
if __name__ == '__main__':
    ...
    model = torch.nn.DataParallel(EAIStereo(args), device_ids=[0])
